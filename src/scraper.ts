...
export class DrivingTestScraper {
  ...
  async login(): Promise<boolean> {
    ...
    const isAlreadyLoggedIn = await this.page
      .locator('img[title="Logout"]')
      .isVisible()
      .catch(() => false);
    ...
    try {
      ...
      await this.page.waitForSelector('h2:has-text("Login")', {
        timeout: 10000,
      });

      // Now locate input fields
      const holderNumberInput = this.page.locator('input#candidateId');
      const birthdateInput = this.page.locator(
        'input[placeholder="12.09.1985"]',
      );
      ...
      await this.page.waitForSelector('img[title="Logout"]', {
        timeout: 15000,
      });
  ...
  async navigateToAppointmentSelection(): Promise<boolean> {
    ...
    try {
      ...
      const isOnSelectionPage = await this.page
        .locator('h2:has-text("Neuer Termin")')
        .isVisible()
        .catch(() => false);
      ...
      const auswahlenVisible = await this.page
        .getByText('Auswählen', { exact: true })
        .isVisible()
        .catch(() => false);
      ...
      if (auswahlenVisible) {
        ...
        const auswahlenCell = await this.page
          .locator('td:has-text("Auswählen")')
          .first();
        if (await auswahlenCell.isVisible()) await auswahlenCell.click();
        else throw new Error('Could not find Auswählen button');
      }

      // Wait for the selection page to load
      await this.page.waitForSelector('h2:has-text("Neuer Termin")', {
        timeout: 10000,
      });
    ...
    try {
      ...
      for (let week = 0; week < 6; week++) {
        ...
        if (week > 0) {
          ...
          const nextButtonDisabled = await nextButton
            .isDisabled()
            .catch(() => true);
        ...
        const dateHeaders = await this.page
          .locator('#jour h3')
          .allTextContents();

        console.log(
          `Week ${week + 1}: Found ${dayNames.length} day names and ${dateHeaders.length} dates`,
        );
        ...
        for (
          let day = 0;
          day < Math.min(dayNames.length, dateHeaders.length);
          day++
        ) {
          ...
          try {
            ...
            noAppointmentText = await this.page
              .locator(
                `#jour:nth-child(${dayNumber}) p:has-text("Keine Termine frei")`,
              )
              .isVisible()
              .catch(() => false);
          } catch (e) {
            console.log(
              `Could not determine if appointments are available for ${dayName} ${date}`,
            );
            console.error(e);
          }
          ...
          if (!noAppointmentText) {
            ...
            const timeSlots = await this.page
              .locator(
                `#jour:nth-child(${dayNumber}) .hour button:not([disabled])`,
              )
              .evaluateAll((buttons) =>
            ...
            if (validTimeSlots.length > 0) {
              console.log(
                `Found ${validTimeSlots.length} slots for ${dayName} ${date}: ${validTimeSlots.join(', ')}`,
              );
  ...
  async checkAllLocations(): Promise<AppointmentResults[]> {
    ...
    const onAppointmentPage = await this.page
      .getByText('Neuer Termin')
      .isVisible()
      .catch(() => false);
